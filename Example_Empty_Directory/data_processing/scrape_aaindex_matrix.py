...
def scrape_aaindex_indicies() -> List[str]:
    """Web scraper designed to extract list of property matrix names from AAINDEX

    Returns:
        List[str]: names of all of the accession numbers on AAINDEX website
    """
...
def find_accession_numbers(clean_names: List[str]) -> pd.DataFrame:
    """extracts information from all amino acid property matricies on AAINDEX and saves to df

    Args:
        clean_names (List[str]): output from scrape_aaindex_indicies

    Returns:
        pd.DataFrame: dataframe that contains functional information for every amino acid across every matrix they host on their site
    """
    # find all accession numbers located on AAindex and save to list
    AA_list = [
        "A",
